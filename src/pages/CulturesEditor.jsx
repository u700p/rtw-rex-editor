...
function buildExpandedBiText(cultures) {
  const store = getTextLocalizationStore();
  const expanded = store['expanded_bi.txt'] || store['expanded.txt'];
  const map = expanded?.rawText
    ? parseTextLocFile(expanded.rawText)
    : {};
  ...
  for (const culture of cultures || []) {
    const key = String(culture.name || '').toUpperCase();
    if (key) map[key] = map[key] || cultureDisplayName(culture.name);
  }

  return serializeTextLocFile(map, { header: expanded?.rawText ? undefined : 'Culture display names generated by RTW Rex Editor' });
}
