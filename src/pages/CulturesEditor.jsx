...
import { Globe, FolderOpen, Download, Trash2, Copy, FileText } from 'lucide-react';
...
import { appendTextLocEntries, parseTextLocFile, serializeTextLocFile } from '@/lib/textLocParser';
...
function buildExpandedBiText(cultures) {
  ...
  if (existing.rawText) {
    return appendTextLocEntries(existing.rawText, getCultureStringEntries(cultures)).text;
  }
...
function persistCultureStrings(cultures) {
  ...
  if (existing.rawText) {
    const appended = appendTextLocEntries(existing.rawText, getCultureStringEntries(cultures));
    rawText = appended.text;
    added.push(...appended.added);
    kept.push(...appended.kept);
  } else {
    for (const entry of getCultureStringEntries(cultures)) {
      if (map[entry.key]) {
        kept.push(entry.key);
    ...
    rawText = serializeTextLocFile(map, { header: 'Culture display names generated by RTW Rex Editor' });
  }

  const nextMap = parseTextLocFile(rawText);
