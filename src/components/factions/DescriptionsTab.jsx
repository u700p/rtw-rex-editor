...
import { appendTextLocEntries, parseTextLocFile, serializeTextLocEntries, textLocMapToEntries } from '@/lib/textLocParser';
...
export default function DescriptionsTab({ factionName }) {
  ...
  const persistAddedExpandedEntries = useCallback((entries) => {
    const normalizedEntries = (entries || []).map((entry) => ({
      key: normalizeLocKey(entry.key),
      value: entry.value ?? '',
    })).filter(entry => entry.key);
    const appended = appendTextLocEntries(rawText, normalizedEntries, { header: 'RTW faction strings generated by RTW Rex Editor' });
    const storedEntries = textLocMapToEntries(parseTextLocFile(appended.text))
      .map((entry) => ({ key: normalizeLocKey(entry.key), value: entry.value ?? '' }));
    updateTextLocalizationFile(EXPANDED_BI_FILE, {
      entries: storedEntries,
    ...
    setRawText(appended.text);
    try { localStorage.setItem(GLOBAL_STRINGS_KEY, JSON.stringify({ entries: storedEntries, rawText: appended.text })); } catch {}
  ...
  const loadLocalizationText = useCallback(async (e) => {
    ...
    if (entries.length) {
      ...
      try { localStorage.setItem(GLOBAL_STRINGS_KEY, JSON.stringify({ entries, rawText: text })); } catch {}
    }
    e.target.value = '';
  }, [factionName]);
  ...
  const generateRtwEntries = () => {
    ...
    const factionUpper = factionName.toUpperCase();
    const stored = persistAddedExpandedEntries(updated);
    setAllEntries(stored);
    setLocalizationEntries(stored.filter(entry => entry.key?.toUpperCase().includes(factionUpper)));
  };
  ...
  const confirmCopyFromFaction = () => {
    ...
    const stored = persistAddedExpandedEntries(updated);
    setAllEntries(stored);
    setLocalizationEntries(stored.filter(entry => entry.key?.toUpperCase().includes(dstUpper)));
